...
const GameCard = ({ game }: Props) => {
  return (
    <Card _hover={{transform:'scale(1.03)', transition:'transform .15s ease-in'}} borderRadius={10} overflow={"hidden"} width='100%' height={310}>
      <Image src={getCroppedImageUrl(game.background_image)}></Image>
      <CardBody>
        <Heading fontSize={"2xl"} marginBottom={2}>       <Link to={'/games/'+game.slug}>{game.name}</Link>
</Heading>
